    ...
    public void processResult(Context context, int result, byte[] response, int httpStatusCode) {
        ...
        if (pendingIntent != null) {
            ...
            if (response != null) {
                ...
                SendConf aa = (SendConf)new PduParser(response).parse();
                int status = aa.getResponseStatus();

                String value = new String(response, Charset.forName("EUC-KR"));
                //String byteToString = new String(response,0,response.length);
                //Log.d("", value);
            }
            if (messageUri != null) {
